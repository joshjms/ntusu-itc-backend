...
class EventListMatricTestCase(BaseAPITestCase):
    @classmethod
    ...
    def test_event_list_matric_success(self):
        # Try as an event creator
        self.newclient1.force_authenticate(user = self.newuser1)
        url = reverse('event:event-list-matric', args=(self.event1.pk,))
        resp = self.newclient1.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data[0]["officer_name"], "Bob")
        self.assertEqual(resp.data[0]["event"],self.event1.pk)
        self.assertEqual(resp.data[0]["matric_number"], "U11111111")
        
        self.assertEqual(resp.data[1]["officer_name"], "Bob")
        self.assertEqual(resp.data[1]["event"],self.event1.pk)
        self.assertEqual(resp.data[1]["matric_number"], "U22222222")

    def test_event_list_matric_fail(self):
